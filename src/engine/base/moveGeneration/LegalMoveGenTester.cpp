...
void LegalMoveGenTester::runTest(const Test &test) {
    ...
    for (uint32_t i = 0; i < test.nodes.size(); i = i + 1) {
        ...
        if (correct == got) {
            std::cout << "Depth " << std::setw(4) << i << ". Correct: " << std::setw(18) << correct << ". Got: " << std::setw(18) << got << ". Speed: " << std::setw(10) << speed << "kk nodes per second. OK." << std::endl;
        }
        else {
            std::cout << "Depth " << std::setw(4) << i << ". Correct: " << std::setw(18) << correct << ". Got: " << std::setw(18) << got << ". Speed: " << std::setw(10) << speed << "kk nodes per second. Error." << std::endl;
            std::terminate();
        }
    }
    std::cout << std::endl;
