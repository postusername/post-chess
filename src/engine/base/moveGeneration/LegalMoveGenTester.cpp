...
void LegalMoveGenTester::runTest(const Test &test) {
    auto position = Position(test.shortFen, test.enPassant, test.wlCastling, test.wsCastling, test.blCastling,
                             test.bsCastling, test.side);
    ...
    for (uint32_t i = 0; i < test.nodes.size(); i = i + 1) {
        ...
        float speed = static_cast<float>(got) / (static_cast<float>((nsecs - start)) / static_cast<float>(1e+9)) / static_cast<float>(1e+6);

        if (correct == got) {
            std::cout << "Depth " << std::setw(4) << i << ". Correct: " << std::setw(18) << correct << ". Got: " <<
                    std::setw(18) << got << ". Speed: " << std::setw(10) << speed << "kk nodes per second. OK." <<
                    std::endl;
        } else {
            std::cout << "Depth " << std::setw(4) << i << ". Correct: " << std::setw(18) << correct << ". Got: " <<
                    std::setw(18) << got << ". Speed: " << std::setw(10) << speed << "kk nodes per second. Error." <<
                    std::endl;
            std::terminate();
        }
    }
