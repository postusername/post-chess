...
Pieces::Pieces(const std::string &shortFen) {
    ...
    for (auto buff: shortFen) {
        if (buff == '/') {
            ...
            if (std::isupper(buff)) {
                buff = static_cast<char>(std::tolower(buff));
                side = SIDE::WHITE;
            } else {
                side = SIDE::BLACK;
            }
            ...
            switch (buff) {
                case 'p':
                    this->pieceBitboards[side][PIECE::PAWN] = BOp::set1(this->pieceBitboards[side][PIECE::PAWN],
                                                                        y * 8 + x);
                    break;
                case 'n':
                    this->pieceBitboards[side][PIECE::KNIGHT] = BOp::set1(
                        this->pieceBitboards[side][PIECE::KNIGHT], y * 8 + x);
                    break;
                case 'b':
                    this->pieceBitboards[side][PIECE::BISHOP] = BOp::set1(
                        this->pieceBitboards[side][PIECE::BISHOP], y * 8 + x);
                    break;
                case 'r':
                    this->pieceBitboards[side][PIECE::ROOK] = BOp::set1(this->pieceBitboards[side][PIECE::ROOK],
                                                                        y * 8 + x);
                    break;
                case 'q':
                    this->pieceBitboards[side][PIECE::QUEEN] = BOp::set1(
                        this->pieceBitboards[side][PIECE::QUEEN], y * 8 + x);
                    break;
                case 'k':
                    this->pieceBitboards[side][PIECE::KING] = BOp::set1(this->pieceBitboards[side][PIECE::KING],
                                                                        y * 8 + x);
                    break;
                default:
                    break;
...
std::ostream &operator<<(std::ostream &ostream, const Pieces &pieces) {
