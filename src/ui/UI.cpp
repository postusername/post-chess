    ...
    for (; ;) {
        while (this->window.pollEvent(this->event)) {
            ...
            if (this->event.type == sf::Event::Resized) {
                sf::FloatRect visibleArea(0, 0, static_cast<float>(this->window.getSize().x),
                                          static_cast<float>(this->window.getSize().y));
                this->window.setView(sf::View(visibleArea));
            } else if (this->event.type == sf::Event::KeyPressed and this->event.key.code == sf::Keyboard::Backspace) {
                if (this->positions_history.size() < 3) {
                    sound.setBuffer(*Storage::getPtr()->getSoundBuffer("message"));
                    sound.play();
                } else {
                    this->positions_history.erase(this->positions_history.end() - 2, this->positions_history.end());
                    this->position = this->positions_history.back();
                }
            } else if (this->event.type == sf::Event::MouseButtonPressed and this->getStatus() ==
                ...
                for (int32_t x = 0; x < 8; x = x + 1) {
                    for (int32_t y = 0; y < 8; y = y + 1) {
                        if (mouse.x > static_cast<int32_t>(this->getCellPosition(x, y).x) and
                            mouse.y > static_cast<int32_t>(this->getCellPosition(x, y).y) and
                            mouse.x < static_cast<int32_t>(this->getCellPosition(x, y).x + this->getCellSize().x) and
                            mouse.y < static_cast<int32_t>(this->getCellPosition(x, y).y + this->getCellSize().y)) {
                ...
                if (cell != sf::Vector2i(INT32_MAX, INT32_MAX)) {
                    ...
                    if (static_cast<float>(mouse.x) - this->getCellPosition(cell.x, cell.y).x < this->getCellSize().x / 2) {
                        if (static_cast<float>(mouse.y) - this->getCellPosition(cell.x, cell.y).y < this->getCellSize().y / 2) {
                    ...
                    if (cell == buff) {
                        ...
                        sound.play();
                    } else if (BOp::getBit(this->position.getPieces().getSideBitboard(SIDE::WHITE),
                                           cell.y * 8 + cell.x)) {
                        buff = cell;
                        sound.setBuffer(*Storage::getPtr()->getSoundBuffer("move"));
                        sound.play();
                        ...
                        for (int32_t i = 0; i < moves.getSize(); i = i + 1) {
                            ...
                            if (move.getFrom() == buff.y * 8 + buff.x and move.getTo() == cell.y * 8 + cell.x and
                                ((moves[i].getFlag() != Move::FLAG::PROMOTE_TO_KNIGHT and moves[i].getFlag() !=
                                  Move::FLAG::PROMOTE_TO_BISHOP and moves[i].getFlag() != Move::FLAG::PROMOTE_TO_ROOK
                                  and moves[i].getFlag() != Move::FLAG::PROMOTE_TO_QUEEN) or promotionFlag == moves[i].
                                 getFlag())) {
                                moveId = i;
                                break;
                            }
                        ...
                        if (moveId != -1) {
                            ...
                            sound.setBuffer(*Storage::getPtr()->getSoundBuffer("move"));
                            sound.play();
                            if (moves[moveId].getDefenderType() != Move::NONE or moves[moveId].getFlag() ==
                                Move::FLAG::EN_PASSANT_CAPTURE) {
                                sound.setBuffer(*Storage::getPtr()->getSoundBuffer("capture"));
                                // Этот звук отменяет предыдущий, так как используется общая переменная.
                                sound.play();
                            }
                            if (this->getStatus() == STATUS::WHITE_WON or this->getStatus() == STATUS::DRAW) {
                                ...
                                sound.setBuffer(*Storage::getPtr()->getSoundBuffer("move"));
                                sound.play();
                                if (move.getDefenderType() != Move::NONE or move.getFlag() ==
                                    Move::FLAG::EN_PASSANT_CAPTURE) {
                                    sound.setBuffer(*Storage::getPtr()->getSoundBuffer("capture"));
                                    sound.play();
                                }
...
void UI::drawPieces() {
    for (int32_t x = 0; x < 8; x = x + 1) {
        for (int32_t y = 0; y < 8; y = y + 1) {
            ...
            sprite.setScale(this->getCellSize().x / sprite.getLocalBounds().width,
                            this->getCellSize().y / sprite.getLocalBounds().height);
            sprite.setPosition(this->getCellPosition(x, y));
            if (sf::Vector2i(x, y) == this->buff) {
                sf::Vector2f oldSize = sf::Vector2f(sprite.getLocalBounds().width * sprite.getScale().x,
                                                    sprite.getLocalBounds().height * sprite.getScale().y);
                sprite.setScale(sprite.getScale().x * 1.2f, sprite.getScale().y * 1.2f);
                sf::Vector2f newSize = sf::Vector2f(sprite.getLocalBounds().width * sprite.getScale().x,
                                                    sprite.getLocalBounds().height * sprite.getScale().y);
                sprite.setPosition(sprite.getPosition().x - (newSize.x - oldSize.x) / 2,
                                   sprite.getPosition().y - (newSize.y - oldSize.y) / 2);
            }
    ...
    for (int32_t i = 0; i < moves.getSize(); i = i + 1) {
        if (moves[i].getFrom() == buff.y * 8 + buff.x) {
            ...
            if (moves[i].getAttackerType() == PIECE::PAWN and (y == 0 or y == 7)) {
                ...
                sprite.setScale(this->getCellSize().x / sprite.getLocalBounds().width * 0.5f,
                                this->getCellSize().y / sprite.getLocalBounds().height * 0.5f);
                ...
                sprite.setPosition(sf::Vector2f(this->getCellPosition(x, y).x,
                                                this->getCellPosition(x, y).y + this->getCellSize().y / 2));
                this->window.draw(sprite);

                sprite.setTexture(*Storage::getPtr()->getTexture(side + "Rook"));
                sprite.setPosition(sf::Vector2f(this->getCellPosition(x, y).x + this->getCellSize().x / 2,
                                                this->getCellPosition(x, y).y));
                this->window.draw(sprite);
...
void UI::updateWindowTitle() {
    switch (this->getStatus()) {
        case STATUS::WHITE_TO_MOVE:
            this->window.setTitle(L"chess-cpp — Ваш ход");
            break;
        case STATUS::BLACK_TO_MOVE:
            this->window.setTitle(L"chess-cpp — Компьютер думает...");
            break;
        case STATUS::WHITE_WON:
            this->window.setTitle(L"chess-cpp — Вы победили!");
            break;
        case STATUS::BLACK_WON:
            this->window.setTitle(L"chess-cpp — Вы проиграли...");
            break;
        case STATUS::DRAW:
            this->window.setTitle(L"chess-cpp — Ничья");
            break;
        default:
            break;
    }
...
sf::Vector2f UI::getCellSize() const {
...
sf::Vector2f UI::getCellPosition(int32_t x, int32_t y) const {
    sf::Vector2f size = this->getCellSize();
    return {
        BOARD_MARGIN + size.x * static_cast<float>(x), static_cast<float>(this->window.getSize().y) - BOARD_MARGIN - size.y * static_cast<float>(y + 1)
    };
}


uint8_t UI::getStatus() const {
    ...
    if (this->position.blackToMove()) {
        ...
        if (moves.getSize() == 0) {
            if (PsLegalMoveMaskGen::inDanger(this->position.getPieces(),
                                             BOp::bsf(this->position.getPieces().getPieceBitboard(
                                                 SIDE::BLACK, PIECE::KING)), SIDE::BLACK)) {
    ...
    if (moves.getSize() == 0) {
        if (PsLegalMoveMaskGen::inDanger(this->position.getPieces(),
                                         BOp::bsf(this->position.getPieces().getPieceBitboard(
                                             SIDE::WHITE, PIECE::KING)), SIDE::WHITE)) {
